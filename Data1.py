...
# create a month colum directly for arrivals vs departures
df['month'] = df.apply(
    lambda row: row['current_job.started_at']
    if row['arrival/departure'] == 'arrival' else row['previous_job.ended_at'], axis = 1
).dt.tz_localize(None).dt.to_period('M').dt.start_time # normalize to start of month and remove timezone

# filter to only 2025
df_2025 = df[df['month'].dt.year == 2025].copy() #a copy to avoid warning
